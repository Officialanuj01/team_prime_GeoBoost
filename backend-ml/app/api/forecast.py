...
async def forecast_occupancy(file: UploadFile = File(...)):
    ...
    # Call Vertex AI endpoint on a background thread pool to keep the event loop non-blocking
    predictions = await asyncio.to_thread(
        vertex_service.get_forecast, dates, occupancy, forecast_context
    )
    ...
    # Generate Strategic AI Recommendations on a background thread pool
    if forecast_items:
        ...
        gemini_insights = await asyncio.to_thread(
            gemini_service.generate_strategic_insights,
            dates,
            occupancy,
            forecast_dates,
            forecast_occupancy,
